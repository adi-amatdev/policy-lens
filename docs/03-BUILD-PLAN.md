...
- [ ] Run all commands in `02-SETUP-AND-SCAFFOLD.md`
...
- [ ] `shared/detection.ts`: implement the 3-tier heuristic from the architecture doc
...
- [ ] `shared/storage.ts`: IndexedDB schema per `04-DATA-MODEL-AND-MESSAGING.md`; store
...
- [ ] For every changed section, call the model with the "explain this change" prompt template
      (see `04-DATA-MODEL-AND-MESSAGING.md`) instead of a plain summarize call
...
- [ ] `dashboard/Dashboard.tsx`: query IndexedDB for all domains; render one card per domain with
